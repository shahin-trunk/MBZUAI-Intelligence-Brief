...
const PhraseCard = memo(function PhraseCard({
  ...
  if (scriptIndex === 2) {
    return (
      <div className="flex flex-col items-center justify-center py-12 sm:py-16 animate-in fade-in duration-500">
        <div className="flex flex-col items-center space-y-5 max-w-md">
          <span className="font-ui text-[10px] uppercase tracking-[0.2em] text-gray-500">
            Bridge
          </span>
          <div className="w-12 h-px bg-gradient-to-r from-transparent via-indigo-400/40 to-transparent" />
          <p className="font-body text-[17px] sm:text-[19px] text-gray-400 text-center italic leading-relaxed font-light">
            {phrase.script2}
          </p>
          <div className="w-12 h-px bg-gradient-to-r from-transparent via-indigo-400/40 to-transparent" />
        </div>
      ...
      <div className="flex flex-col items-center py-6 sm:py-8 animate-in fade-in duration-500">
        ...
        <div className="w-full max-w-lg text-center mb-8">
          <p
            ...
            className="font-body text-[28px] sm:text-[34px] lg:text-[38px] text-white font-semibold leading-tight tracking-tight"
          >
            {targetText}
          </p>
          <p className="font-body text-[14px] sm:text-[15px] text-gray-500 mt-3 font-light">
            {englishText}
          </p>
        </div>
        ...
        {phrase.script1 && (
          <div className="w-full px-2">
            <div className="flex items-center gap-2 mb-4">
              <div className="h-4 w-0.5 rounded-full bg-indigo-400/40" />
              <span className="font-ui text-[10px] uppercase tracking-[0.15em] text-gray-500">
                Teacher explains
              </span>
            </div>
            <PhraseHighlightText
        ...
        {showGrammarTrigger && phrase.script4 && (
          <button
            ...
            className="mt-8 group inline-flex items-center gap-2 px-4 py-2 rounded-full border border-indigo-500/20 text-indigo-400/70 hover:text-indigo-300 hover:border-indigo-500/40 transition-all duration-200 text-xs font-ui"
          >
  ...
  return (
    <div className="flex flex-col items-center py-6 sm:py-10 animate-in fade-in duration-500">
      ...
      {/* English reference */}
      <p className="font-body text-[13px] text-gray-500 italic font-light text-center mb-6">
        {englishText}
      </p>
      ...
      {phrase.grammar.key_words && phrase.grammar.key_words.length > 0 && (
        <div className="flex flex-wrap gap-2 justify-center mb-5 max-w-sm">
          {phrase.grammar.key_words.map((kw, i) => (
            ...
            >
              <span
                className="font-body text-[12px] text-indigo-300 font-medium"
      ...
      {phrase.grammar.phonetic_features && (
        <div className="px-4 py-2.5 rounded-lg bg-indigo-500/5 border border-indigo-500/10 mb-4 max-w-sm text-center">
      ...
      {phrase.grammar.phonetic_guide && (
        <code className="font-mono text-[11px] text-gray-500 mb-4 text-center block">
          {phrase.grammar.phonetic_guide}
        </code>
      )}
      ...
      {phrase.grammar.cognate_note && (
        <div className="px-4 py-2.5 rounded-lg bg-emerald-500/5 border border-emerald-500/10 mb-4 max-w-sm text-center">
          <p className="font-body text-[11px] text-gray-400 leading-relaxed">
            {phrase.grammar.cognate_note}
          </p>
        </div>
      ...
      {showGrammarTrigger && phrase.script4 && (
        <button
          onClick={onExpandGrammar}
          className="mt-2 group inline-flex items-center gap-2 px-4 py-2 rounded-full border border-indigo-500/20 text-indigo-400/70 hover:text-indigo-300 hover:border-indigo-500/40 transition-all duration-200 text-xs font-ui"
        >
          <Mic className="h-3.5 w-3.5" strokeWidth={1.5} />
          <span>Grammar deep dive</span>
        </button>
