...
import type { FlagshipProject, FlagshipProjectStepStatus } from "@/lib/types/internal-intelligence";

interface FlagshipProjectCardsProps {
  projects: FlagshipProject[];
}

const STATUS_STYLES: Record<string, string> = {
  "In progress": "bg-accent-primary/15 text-accent-primary border-accent-primary/30",
  "In production": "bg-sig-high/15 text-sig-high border-sig-high/30",
  "In planning": "bg-bg-tertiary text-text-muted border-border-primary",
};

const STEP_ICON: Record<FlagshipProjectStepStatus, { symbol: string; color: string }> = {
  completed: { symbol: "\u2713", color: "text-accent-success" },
  in_progress: { symbol: "\u25CF", color: "text-sig-high" },
  pending: { symbol: "\u25CB", color: "text-text-muted" },
};

export function FlagshipProjectCards({ projects }: FlagshipProjectCardsProps) {
